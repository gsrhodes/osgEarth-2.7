...
#define VP_MIN_DURATION      2.0     // minimum fly time. GAJ Changed from 2.0
#define VP_METERS_PER_SECOND 2500.0  // fly speed
#define VP_MAX_DURATION      2.0 //8.0     // maximum fly time. GAJ Changed from 2.0
                    ...
                    Container*           userContainer ) const
{
    ...
    for( int i=0; i<args.argc(); ++i )
    {
        ...
        {
#ifdef _DEBUG
			std::string nodeName = args[i];
            node = osgDB::readNodeFile( nodeName, options );
#else
